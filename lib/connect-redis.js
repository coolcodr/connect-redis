...
module.exports = function (session) {
  ...
  RedisStore.prototype.get = function (sid, fn) {
    ...
    store.client.get(psid, function (er, data) {
      ...
      var result = {};
      try {
        var djangoSession = new Buffer(data, "base64").toString()
        djangoSession = djangoSession.substring(djangoSession.indexOf(":") + 1)
        result = JSON.parse(djangoSession);
        result.cookie = {};
      } catch (er) {
        return fn(er);
      }
  ...
  RedisStore.prototype.set = function (sid, sess, fn) {
    debug('update session is not supported.');
